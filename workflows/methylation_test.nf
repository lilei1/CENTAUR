...
/*
 * CENTAUR Methylation Test Workflow
 * Simplified test workflow for methylation analysis
 */
...
params {
    test_mode = true
    input_dir = "$baseDir/data/test_data/methylation"
    sample_sheet = "$baseDir/data/test_data/sample_sheet_test.csv"
    output_dir = "$baseDir/data/test_results"
}
    ...
    Channel.fromPath(params.sample_sheet)
        ...
        .map { row -> 
            [
                ...
                row.stage,
                row.sample_type,
                "${params.input_dir}/${row.sample_id}/${row.sample_id}_R1.fastq.gz"
            ]
        }
        .set { sample_channel }
...
workflow.onComplete {
    ...
    Test results are available in: ${params.output_dir}
    """
}
