#!/bin/bash -ue
echo "Processing RNA-seq data for RNA_TEST_003" > RNA_TEST_003_rnaseq_test.txt
echo "Sample type: Cancer" >> RNA_TEST_003_rnaseq_test.txt
echo "Cancer type: Lung" >> RNA_TEST_003_rnaseq_test.txt
echo "Stage: Late" >> RNA_TEST_003_rnaseq_test.txt
echo "Input file: RNA_TEST_003_R1.fastq.gz" >> RNA_TEST_003_rnaseq_test.txt
echo "Processing completed at: $(date)" >> RNA_TEST_003_rnaseq_test.txt
...
counts_df.to_csv('RNA_TEST_003_gene_counts.txt', sep='	', index=False)

# Calculate immune signatures
immune_signatures = {
    'sample_id': 'RNA_TEST_003',
...
immune_df.to_csv('RNA_TEST_003_immune_signatures.tsv', sep='	', index=False)

print(f'Generated gene expression data for RNA_TEST_003')
