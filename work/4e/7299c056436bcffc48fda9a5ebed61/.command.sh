#!/bin/bash -ue
echo "Training NaiveBayes model" > NaiveBayes_model_results.txt
    echo "Training completed at: $(date)" >> NaiveBayes_model_results.txt
...
print(f'Training NaiveBayes model')
...
# Performance metrics
performance = simulate_ml_training('NaiveBayes', n_features)
performance_df = pd.DataFrame([performance])
performance_df.to_csv('NaiveBayes_performance_metrics.tsv', sep='	', index=False)

# Feature importance
feature_importance = simulate_feature_importance('NaiveBayes', n_features)
feature_importance_df = pd.DataFrame(feature_importance)
feature_importance_df.to_csv('NaiveBayes_feature_importance.tsv', sep='	', index=False)

# Predictions
predictions = simulate_predictions('NaiveBayes', n_samples)
predictions_df = pd.DataFrame(predictions)
predictions_df.to_csv('NaiveBayes_predictions.tsv', sep='	', index=False)

print(f'NaiveBayes model training completed')
