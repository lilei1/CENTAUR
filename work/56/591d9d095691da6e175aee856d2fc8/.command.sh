#!/bin/bash -ue
echo "Processing RNA-seq data for RNA_TEST_002" > RNA_TEST_002_rnaseq_test.txt
echo "Sample type: Control" >> RNA_TEST_002_rnaseq_test.txt
echo "Cancer type: CRC" >> RNA_TEST_002_rnaseq_test.txt
echo "Stage: Early" >> RNA_TEST_002_rnaseq_test.txt
echo "Input file: RNA_TEST_002_R1.fastq.gz" >> RNA_TEST_002_rnaseq_test.txt
echo "Processing completed at: $(date)" >> RNA_TEST_002_rnaseq_test.txt
...
counts_df.to_csv('RNA_TEST_002_gene_counts.txt', sep='	', index=False)

# Calculate immune signatures
immune_signatures = {
    'sample_id': 'RNA_TEST_002',
...
immune_df.to_csv('RNA_TEST_002_immune_signatures.tsv', sep='	', index=False)

print(f'Generated gene expression data for RNA_TEST_002')
