#!/bin/bash -ue
echo "Processing WES data for WES_TEST_003" > WES_TEST_003_wes_test.txt
    echo "Sample type: Cancer" >> WES_TEST_003_wes_test.txt
    echo "Cancer type: Lung" >> WES_TEST_003_wes_test.txt
    echo "Stage: Late" >> WES_TEST_003_wes_test.txt
    echo "Tumor file: WES_TEST_003_tumor_R1.fastq.gz" >> WES_TEST_003_wes_test.txt
    echo "Normal file: WES_TEST_003_normal_R1.fastq.gz" >> WES_TEST_003_wes_test.txt
    echo "Processing completed at: $(date)" >> WES_TEST_003_wes_test.txt
...
'''
    ...
    with open('WES_TEST_003_somatic_variants.vcf', 'w') as f:
        f.write(vcf_content)
    ...
    with open('WES_TEST_003_tmb.txt', 'w') as f:
        f.write(f'{tmb:.2f}')
    ...
    features_df.to_csv('WES_TEST_003_variant_features.tsv', sep='	', index=False)
    
    print(f'Generated WES analysis for WES_TEST_003')
