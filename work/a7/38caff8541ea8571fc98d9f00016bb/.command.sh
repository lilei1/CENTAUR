#!/bin/bash -ue
echo "Starting biomarker discovery for ML_TEST_001" > ML_TEST_001_biomarker_discovery.txt
    echo "Sample type: Cancer" >> ML_TEST_001_biomarker_discovery.txt
    echo "Cancer type: CRC" >> ML_TEST_001_biomarker_discovery.txt
    echo "Stage: Early" >> ML_TEST_001_biomarker_discovery.txt
    echo "Processing completed at: $(date)" >> ML_TEST_001_biomarker_discovery.txt
...
print(f'Starting biomarker discovery for ML_TEST_001')
...
stats_df.to_csv('ML_TEST_001_statistical_tests.tsv', sep='	', index=False)
...
feature_importance_df.to_csv('ML_TEST_001_feature_importance.tsv', sep='	', index=False)

print(f'Biomarker discovery completed for ML_TEST_001')
