#!/bin/bash -ue
echo "Processing methylation data for TEST_002" > TEST_002_methylation_test.txt
echo "Sample type: Control" >> TEST_002_methylation_test.txt
echo "Cancer type: CRC" >> TEST_002_methylation_test.txt
echo "Stage: Early" >> TEST_002_methylation_test.txt
echo "Input file: TEST_002_R1.fastq.gz" >> TEST_002_methylation_test.txt
echo "Processing completed at: $(date)" >> TEST_002_methylation_test.txt
...
df.to_csv('TEST_002_fragmentomics_test.tsv', sep='	', index=False)
print(f'Generated fragmentomics features for TEST_002')
"
